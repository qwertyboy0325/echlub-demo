...
import type { BrainId, PerformanceScriptEvent } from "./types";
...
export class PresentationEngine {
  private cursors = new Map<BrainId, HTMLElement>();

  initialize(): void {
    (["memory", "pulse", "blend", "story"] as BrainId[]).forEach((brain) => {
      const cursor = document.querySelector<HTMLElement>(`[data-cursor="${brain}"]`);
      if (cursor) this.cursors.set(brain, cursor);
    });
  }
  ...
  playEvent(event: PerformanceScriptEvent): void {
    const panel = document.querySelector<HTMLElement>(`[data-brain="${event.brain}"]`);
    ...
    const target = event.target
      ? panel.querySelector<HTMLElement>(`[data-target="${event.target}"]`)
      : panel.querySelector<HTMLElement>(".brain-primary-target");
