...
import { initialDrafts, scenes, sceneForBar, TOTAL_BARS } from "./musicData.js";
import { jamMemories, performanceScript } from "./performanceScript.js";
...
app.innerHTML = `
  <main class="app-shell">
    ...
    <section class="brain-grid">
      ${["memory", "pulse", "blend", "story"].map(renderBrainWindow).join("")}
    </section>
...
`;
function renderBrainWindow(brain) {
...
function renderBrainWorkspace(brain) {
    if (brain === "memory") {
...
startButton.addEventListener("click", async () => {
    if (!initialized) {
        ...
        scheduleScript();
        initialized = true;
        document.querySelector("#audio-status").textContent = "audio ready";
        document.querySelector("#audio-status").classList.add("live");
    }
    resetRuntime();
    audioEngine.start();
    isPaused = false;
...
function scheduleScript() {
    ...
    scriptIds = performanceScript.map((event) => transport.schedule((time) => {
        Tone.getDraw().schedule(() => applyScriptEvent(event), time);
    }, event.at));
}
function applyScriptEvent(event) {
    ...
    if (event.action === "queue" && event.target) {
        if (!state.queue.includes(event.target))
            state.queue.push(event.target);
    }
    ...
    if (event.action === "filter" && typeof event.value === "number") {
        const knob = document.querySelector("#filter-knob");
        if (knob)
            knob.style.transform = `rotate(${Math.min(135, -120 + event.value / 15)}deg)`;
    }
    if (event.action === "delay" && typeof event.value === "number") {
        const knob = document.querySelector("#delay-knob");
        if (knob)
            knob.style.transform = `rotate(${-120 + event.value * 260}deg)`;
    }
    if (event.action === "memory")
        captureAvailableMemories();
    renderState();
}
function updateDraftState(event) {
    ...
    if (nextStatus)
        state.drafts[event.target].status = nextStatus;
}
function activateScene(scene) {
    ...
    document.querySelector("#next-scene").textContent = scenes[index + 1]?.title ?? "Ending silence";
    presentation.pulseScene(scene.id);
    captureAvailableMemories();
}
function captureAvailableMemories() {
    ...
    jamMemories.forEach((memory) => {
        ...
        if (transportBar >= requiredBar && !state.history.some((item) => item.id === memory.id)) {
            ...
            const button = document.querySelector(`[data-memory="${memory.id}"]`);
            if (button) {
    ...
    document.querySelector("#memory-count").textContent = `${state.history.length} / ${jamMemories.length}`;
}
function updateTransportUi() {
    ...
    document.querySelector("#position").textContent = `${String(state.currentBar + 1).padStart(2, "0")} · ${state.currentBeat + 1} · ${state.currentSixteenth + 1}`;
...
function updateActionUi(event) {
    document.querySelector("#action-label").textContent = event.label;
    document.querySelector("#action-detail").textContent = event.detail;
    document.querySelector("#action-actor").textContent = brainMeta[event.brain].title;
    document.querySelector(`#${event.brain}-thought`).textContent = event.detail;
    document.querySelectorAll(".brain-state").forEach((element) => { element.textContent = "observing"; });
    document.querySelector(`#${event.brain}-state`).textContent = event.action;
}
function renderState() {
    document.querySelector("#queue-count").textContent = `${state.queue.length} decision${state.queue.length === 1 ? "" : "s"}`;
    Object.values(state.drafts).forEach((draft) => {
        const target = document.querySelector(`[data-target="${draft.id}"]`);
        if (!target)
            return;
...
function resetRuntime() {
    ...
    document.querySelectorAll("[data-memory]").forEach((button) => { button.disabled = true; button.classList.remove("captured"); });
    document.querySelector("#memory-count").textContent = `0 / ${jamMemories.length}`;
    document.querySelector("#action-label").textContent = "Performance started";
    document.querySelector("#action-detail").textContent = "The four scripted brains are preparing their private drafts.";
    document.querySelector("#action-actor").textContent = "System";
    presentation.reset();
...
function finishPerformance() {
    ...
    document.querySelector("#action-label").textContent = "Arrangement captured";
    document.querySelector("#action-detail").textContent = "The same 40-bar result can now be explained as four independent streams of musical decisions.";
    document.querySelector("#action-actor").textContent = "System";
}
